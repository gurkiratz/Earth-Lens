...
async function getPlaceDetails(placeId) {
  try {
    ...
    if (!response.ok) {
      ...
      console.error(
        'Place Details response error:',
        response.status,
        response.statusText,
        errorText
      )
      throw new Error(
        `Failed to get place details: ${response.statusText} - ${errorText}`
      )
    }
    const data = await response.json()
    ...
    if (!geocodeResponse.ok) {
      ...
      console.error(
        'Geocoding response error:',
        geocodeResponse.status,
        geocodeResponse.statusText,
        errorText
      )
      throw new Error(
        `Failed to get geocode: ${geocodeResponse.statusText} - ${errorText}`
      )
    }
...
function distanceFromPointToSegment(point, segStart, segEnd) {
  ...
  const projection = Math.max(
    0,
    Math.min(1, (A * A - C * C + B * B) / (2 * B * B))
  )
  const closestPoint = google.maps.geometry.spherical.interpolate(
    segStart,
    segEnd,
    projection
  )
...
function computeDetourWaypoint(zone, referencePoint) {
  ...
  if (d === 0) {
    ...
    return new google.maps.LatLng(
      zone.lat + (zone.radius + 100) / 111111,
      zone.lng
    )
  }
  ...
  const factorLng =
    (zone.radius + 100) / (111111 * Math.cos((zone.lat * Math.PI) / 180))
  return new google.maps.LatLng(
    zone.lat + (dx / d) * factorLat,
    zone.lng + (dy / d) * factorLng
  )
}
...
async function calculateRoute() {
  ...
  const originLat =
    typeof originPlace.lat === 'function' ? originPlace.lat() : originPlace.lat
  const originLng =
    typeof originPlace.lng === 'function' ? originPlace.lng() : originPlace.lng
  ...
  while (iteration < MAX_ITERATIONS) {
    ...
    const requestBody = {
      ...
      destination: {
        location: {
          latLng: { latitude: destLatLng.lat(), longitude: destLatLng.lng() },
        },
      },
      travelMode: 'DRIVE',
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(`Route computation failed: ${response.statusText}`)
      ...
      waypoints = generateAvoidanceWaypoints(
        decodedPath,
        MIN_BUFFER,
        originLatLng
      ) // Pass originLatLng here

      if (iteration === MAX_ITERATIONS) {
        alert(
          'No route found that completely avoids all zones by ' +
            (MIN_BUFFER + avoidZones[0].radius) +
            ' meters.'
        )
        return
      }
...
function generateAvoidanceWaypoints(decodedPath, buffer, originLatLng) {
  ...
  for (const zone of avoidZones) {
    ...
    if (minDistance < zone.radius + buffer) {
      ...
      waypoints.push({
        location: {
          latLng: { latitude: detour1.lat(), longitude: detour1.lng() },
        },
      })
      waypoints.push({
        location: {
          latLng: { latitude: detour2.lat(), longitude: detour2.lng() },
        },
      })
    }
  }
  ...
  waypoints.sort((a, b) => {
    const distA = google.maps.geometry.spherical.computeDistanceBetween(
      ...
      new google.maps.LatLng(
        a.location.latLng.latitude,
        a.location.latLng.longitude
      )
    )
    const distB = google.maps.geometry.spherical.computeDistanceBetween(
      originLatLng,
      new google.maps.LatLng(
        b.location.latLng.latitude,
        b.location.latLng.longitude
      )
    )
    return distA - distB
  })
